...
class Medusa extends mixin(EventEmitter, Component) {
  ...
  initStage () {
    ...
    const depthMaterialDummy = new MeshDepthMaterial()
    ...
    this.depthMaterial.depthPacking = depthMaterialDummy.depthPacking
  ...
  initControls () {
    // this.controls = new OrbitControls(this.camera, document.getElementById('header-image'))
  ...
  setControlsSettings () {
    ...
    // this.controls.target = new Vector3(-580, 50, 0)
  ...
  renderFrame () {
    ...
    if (this.FDG) {
      ...
      if (this.FDG.outputPositionRenderTarget) {
        this.depthMaterial.uniforms.positionTexture.value = this.FDG.outputPositionRenderTarget.texture
        this.depthMaterial.uniforms.camPos.value = this.FDG.nodes.material.uniforms.camPos.value
        this.depthMaterial.uniforms.isDepth.value = 1.0
      }
    }
